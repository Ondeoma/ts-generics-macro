...
function extractTypeMap(context: ContextBag, macroCall: MacroCallExpression): Map<ts.Symbol, ts.TypeNode> | undefined {
  ...
  if (typeParams === undefined || typeArgs === undefined || typeParams.length != typeArgs.length) {
    const diag = createDiagnosticForMacroCall(macroCall.callExpression, DiagnosticMessage.MacroCallTypeArgsMismatch);
  ...
  if (typeParamSymbols.length < typeParams.length) {
    const diag = createDiagnosticForMacroCall(macroCall.callExpression, DiagnosticMessage.MacroTypeParamWithNoSymbol);
    context.extra.addDiagnostic(diag);
    return undefined;
  }
...
function createMacroExpansionVisitor(context: ContextBag, macroMap: MacroMap): ts.Visitor {
  const visitor: ts.Visitor = (node: ts.Node) => {
    const macroCall = extractMacroCallExpression(node, macroMap, context.checker);
    if (!macroCall) {
      return ts.visitEachChild(node, visitor, context.transformer);
    }
    ...
    const modifiers = macroCall.macroDefinition.modifiers?.filter(mod => mod.kind === ts.SyntaxKind.AsyncKeyword)
